...
citations = citations.select('id', 'title').distinct()
...
base_features = base_features.join(
    citations, citations.title == base_features.page_title, how='inner'
)
...
filtered = dataset_citations.join(
    ...
    (base_features.page_id == dataset_citations.id) &
        (base_features.retrieved_citation == dataset_citations.citation),
    how='inner'
)
